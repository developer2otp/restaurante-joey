...
var editarProducto = function(id_prod){
    ...
    $.ajax({
        type: "POST",
        url: $('#url').val()+"ajuste/producto_listSolo",
        data: {
            id_prod: id_prod
        },
        dataType: "json",
        success: function(item){
            ...
            if(campo.id_tipo == 1){
                ...
                $("input[name=id_tipo][value='0']").attr("checked",false);
            } else if (campo.id_tipo == 0){
                $('#ntransf').addClass('active');
                $('#transf').removeClass('active');
                $("input[name=id_tipo][value='1']").attr("checked",false);
                $("input[name=id_tipo][value='0']").attr("checked",true);
            }
            if(campo.delivery == 1){
    ...
    $.ajax({
        ...
        success: function(data){
            if(data == 1){
                ...
                var icono = 'success';
                var message = 'Datos eliminados correctamente';
                listarCategorias();
            }else if(data == 0){
                var titulo = 'Proceso No Culminado';
                var icono = 'error';
                var message = 'Datos protegidos';
            }
            Swal.fire({   
                title: titulo,   
                text: message,
            ...
            $.ajax({
                ...
                success: function (cod) {
                    if(cod == 0){
                        var titulo = 'Proceso No Culminado';
                        var icono = 'error';
                        var message = 'Datos duplicados';
                    } else if(cod == 1){
                        var titulo = 'Proceso Terminado';
                        var icono = 'success';
                        var message = 'Datos registrados correctamente';
