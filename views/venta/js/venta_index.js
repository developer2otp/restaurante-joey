            ...
            $.each(item['mesa'], function(i, mesa) {
                ...
                if($('#rol_usr').val() == 5){
                    ...
                    if(mesa.id_salon == cod && mesa.estado == 'a'){
                        $('.list-mesas')
                            .append($('<button class="btn btn-green dim btn-large-dim"'
                                +'onclick="registrarMesa('+mesa.id_mesa+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\');">'+mesa.nro_mesa+'<br><img src="public/images/other_icon/unknow.png" width="40px" height="40px"/><input type="radio" autocomplete="off"></button>'));
                        ...
                        $('.list-mesas')
                            ...
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                ...
                                +'<span class="span-a"><i class="far fa-user"></i> '+mesa.nro_personas+'</span><span>'+mesa.nro_mesa+'</span><br><img src="public/images/other_icon/full.png" width="40px" height="40px"/>'
                    ...
                    if(mesa.id_salon == cod && mesa.estado == 'a'){
                        $('.list-mesas')
                            .append($('<button class="btn btn-green dim btn-large-dim"'
                                +'onclick="nuevoPedidoMesa('+mesa.id_mesa+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'+mesa.nro_mesa+'<br><img src="public/images/other_icon/unknow.png" width="40px" height="40px"/><input type="radio" autocomplete="off"></button>'));
                    //mesa en proceso de pago
                    } else if(mesa.id_salon == cod && mesa.estado == 'p'){
                        $('.list-mesas')
                            $('.list-mesas')
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<button class="btn btn-blue dim btn-large-dim" onclick="listarPedidos('+mesa.id_salon+','+mesa.id_mesa+',1,'+mesa.id_pedido+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'
                                +'<span class="span-a"><i class="far fa-user"></i> '+mesa.nro_personas+'</span><span>'+mesa.nro_mesa+'</span><br><img src="public/images/other_icon/full.png" width="40px" height="40px"/>'
                                +'<span class="span-b"><i class="ti-timer"></i> <span class="hora-mesa'+count+++'">'+moment(mesa.fecha_pedido).fromNow(true)+'</span></span><input type="radio" autocomplete="off">'
                                +'</button>'));
                                //mesa ocupada
                    } else if(mesa.id_salon == cod && mesa.estado == 'i'){
                        $('.list-mesas')
                            .append($('<input type="hidden" name="tiempo_mesa[]" value="'+mesa.fecha_pedido+'"/>'
                                +'<button class="btn btn-reed dim btn-large-dim" onclick="listarPedidos('+mesa.id_salon+','+mesa.id_mesa+',1,'+mesa.id_pedido+',\''+mesa.nro_mesa+'\',\''+mesa.desc_salon+'\')">'
                                +'<div class="estado-mesas">'
                                +'<div class="datos-mesas"></div>'
                                +'</div>'
                                +'<span class="span-a"><i class="far fa-user"></i> '+mesa.nro_personas+'</span><span>'+mesa.nro_mesa+'</span><br><img src="public/images/other_icon/full.png" width="40px" height="40px"/>'
                                +'<span class="span-b"><i class="ti-timer"></i> <span class="hora-mesa'+count+++'">'+moment(mesa.fecha_pedido).fromNow(true)+'</span></span><input type="radio" autocomplete="off">'
                                +'</button>'));
...
var mostrador_list_a = function(){
    ...
    var table = $('#list-mostrador-confirmacion').DataTable({
        ...
        "ajax": {
            ...
            "dataType": "json", // Especifica el tipo de dato que esperas recibir del servidor
            "data": {
                estado: 'a'
            }
        },         
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.length != 0) {
                ...
                $('.totalPagar').html('<div><span>'+moneda+' '+formatNumber(total)+'</span>');
            } else {
