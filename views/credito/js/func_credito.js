...
var listar = function(){
	...
	.DataTable({
		...
		"columns":[
            ...
            {"data":"Amortizado","render": function ( data, type, row) {
                return moneda+' '+formatNumber(data);
            }},
            {"data":null,"render": function ( data, type, row) {
                var cal = (data.total - data.Amortizado).toFixed(2);
                return '<span class="label label-warning font-12 p-5"> '+moneda+' ' +formatNumber(cal)+'</span>';
            }},
            {"data":null,"render": function ( data, type, row ) {
                var call = (data.total - data.Amortizado).toFixed(2);
    ...
    $.ajax({
        ...
        success: function(item){
            $.each(item.data, function(i, campo) {
                var call = (campo.total - campo.Amortizado).toFixed(2);
                $('#id_credito').val(campo.id_credito);
                $('#total_credito').val(campo.total);
                $('#monto_amortizado').val(campo.Amortizado);
