    ...
    ?>
     <tr>
            ...
            if ($row->estado == 'i') : 
                ...
                $total_exportation = '0';
                $total_taxed = $total_ope_gravadas - number_format((($descuento/1.18)), 2, ".", "");
                $total_exonerated = ($total_ope_exoneradas > 0 )? ($total_ope_exoneradas + $comision_delivery) : '0';
                $total_unaffected = '0';
                $total_plastic_bag_taxes = '0';    
                $total_igv = $total_igv_gravadas - number_format(($descuento -($descuento/1.18)), 2, ".", "");
                $total = ($total_ope_exoneradas > 0 )? $total_taxed + $total_igv + $total_exonerated : $total_taxed + $total_igv + $total_exonerated + $comision_delivery; // temporal 
                // $total = $row->total;
            endif; 
