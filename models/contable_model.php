...
class Contable_Model extends Model
{
    ...
    public function validador_list($data)
    {
        try
        {
            ...
            if (($data['num_inicial']) && ($data['num_final'])){ $numero = "AND v.nro_doc >= '".$data['num_inicial']."' AND v.nro_doc <= '".$data['num_final']."' "; }

            elseif(($data['num_inicial']) && ($data['num_final']) == ''){ $numero = "AND v.nro_doc >= '".$data['num_inicial']."' "; }

            else {$numero = ""; }
            $ifecha = date('Y-m-d',strtotime($data['ifecha']));
            $ffecha = date('Y-m-d',strtotime($data['ffecha']));
            $stm = $this->db->prepare("SELECT v.*,IFNULL((v.total+v.comis_del-v.desc_monto),0) AS total FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND ".$estado." ".$numero." GROUP BY v.id_ven");
