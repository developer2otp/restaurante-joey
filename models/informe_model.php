...
{
    private $igv_universal;
    public function __construct()
    {
        parent::__construct();
        $this->igv_universal = Session::get('igv')/100;
    }
    ...
    private function getSingleResult($query, $param)
    {
        $result = $this->db->prepare($query);
        $result->execute(array($param));
    ...
    public function venta_all_list()
    {
        ...
        {   
            ...
            $stm = $this->db->prepare("
                    SELECT v.id_ven, v.id_ped, v.id_tped, v.id_tpag, v.pago_efe, v.pago_tar, v.desc_monto, 
                        v.comis_tar, v.comis_del, v.total AS stotal, v.fec_ven, v.desc_td, v.ser_doc, v.nro_doc, 
                        v.estado, CASE WHEN v.estado = 'a' THEN v.pago_efe + v.pago_tar ELSE 0 END AS total, 
                        v.id_cli, v.igv, v.id_usu, v.desc_tipo, v.desc_personal, c.desc_caja, c.id_apc 
                    FROM v_ventas_con AS v 
                    INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc 
                    WHERE (v.fec_ven >= ? AND v.fec_ven <= ?) 
                        AND v.id_tped LIKE ? 
                        AND v.id_tdoc LIKE ? 
                        AND v.id_cli LIKE ? 
                        AND v.estado LIKE ? 
            ...
            foreach ($c as $k => $d) {
                $c[$k]->{'Pedido'} = $this->getSingleResult("SELECT vm.desc_salon, vm.nro_mesa FROM tm_pedido_mesa AS pm INNER JOIN v_mesas AS vm ON pm.id_mesa = vm.id_mesa WHERE pm.id_pedido = ?", $d->id_ped);
                $c[$k]->{'Cliente'} = $this->getSingleResult("SELECT tipo_cliente, dni, ruc, nombre FROM v_clientes WHERE id_cliente = ?", $d->id_cli);
                $c[$k]->{'Personal'} = $this->getSingleResult("SELECT CONCAT(nombres,' ',ape_paterno,' ',ape_materno) AS nombres FROM tm_usuario WHERE id_usu = ?", $d->desc_personal);
                $c[$k]->{'Tipopago'} = $this->getSingleResult("SELECT descripcion AS nombre FROM tm_tipo_pago WHERE id_tipo_pago = ?", $d->id_tpag);
            }
    ...
    public function venta_delivery_list()
    {
        try
        {
            ...
            $stm = $this->db->prepare("
                SELECT v.id_ven, v.id_ped, v.id_cli, v.id_apc, v.desc_td, v.ser_doc, v.nro_doc, 
                    IFNULL((v.total), 0) AS total, v.comis_del, v.fec_ven, d.tipo_entrega, 
                    d.id_repartidor, d.desc_repartidor 
                FROM v_ventas_con AS v 
                INNER JOIN v_pedido_delivery AS d ON v.id_ped = d.id_pedido 
                WHERE (DATE(v.fec_ven) >= ? AND DATE(v.fec_ven) <= ?) 
                    AND d.id_repartidor LIKE ? 
                    AND d.tipo_entrega LIKE ? 
                    AND v.id_tped = 3
            ");
            ...
            foreach ($c as $k => $d) {
                $cliente = $this->getSingleResult("SELECT nombre FROM v_clientes WHERE id_cliente = ?", $d->id_cli);
                $caja = $this->getSingleResult("SELECT desc_caja FROM v_caja_aper WHERE id_apc = ?", $d->id_apc);
    ...
    public function combPro($data)
    {
        try
        {
            $stm = $this->db->selectAll("SELECT * FROM tm_producto WHERE id_catg = :id_catg", ['id_catg' => $data['cod']], PDO::FETCH_OBJ);
    ...
    public function combPre($data)
    {
        try
        {
            $stm = $this->db->selectAll("SELECT * FROM tm_producto_pres WHERE id_prod = :id_prod", ['id_prod' => $data['cod']], PDO::FETCH_OBJ);
    ...
    public function venta_mozo_list($data)
    {
        try
        {
            ...
            $stm = $this->db->prepare("
                SELECT v.fec_ven, v.desc_td, CONCAT(v.ser_doc,'-',v.nro_doc) AS numero,
                IFNULL(SUM(v.pago_efe + v.pago_tar), 0) AS total, v.id_cli, pm.id_mozo
                FROM v_ventas_con AS v
                INNER JOIN tm_pedido_mesa AS pm ON v.id_ped = pm.id_pedido
                WHERE (DATE(v.fec_ven) >= ? AND DATE(v.fec_ven) <= ?)
                AND pm.id_mozo LIKE ?
                AND v.estado = 'a'
                GROUP BY v.id_ven
            ...
            foreach ($c as $k => $d) {
                $mozo = $this->getSingleResult("SELECT CONCAT(nombres,' ',ape_paterno,' ',ape_materno) AS nombre FROM v_usuarios WHERE id_usu = ?", $d->id_mozo);
                $cliente = $this->getSingleResult("SELECT nombre FROM v_clientes WHERE id_cliente = ?", $d->id_cli);
    ...
    public function venta_fpago_list($data)
    {
        try
        {
            ...
            $stm = $this->db->prepare("
                SELECT v.id_ven, v.id_ped, v.id_tpag, v.pago_efe, v.pago_tar, v.desc_monto, 
                    v.comis_tar, v.comis_del, v.total AS stotal, v.fec_ven, v.desc_td, 
                    CONCAT(v.ser_doc, '-', v.nro_doc) AS numero, IFNULL(SUM(v.pago_efe + v.pago_tar), 0) AS total, 
                    v.id_cli, v.igv, v.id_usu, c.desc_caja, v.estado, v.codigo_operacion 
                FROM v_ventas_con AS v 
                INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc 
                WHERE (v.fec_ven >= ? AND v.fec_ven <= ?) 
                    AND v.id_tpag LIKE ? 
            ...
            foreach ($c as $k => $d) {
                $cliente = $this->getSingleResult("SELECT nombre FROM v_clientes WHERE id_cliente = ?", $d->id_cli);
                $tipopago = $this->getSingleResult("SELECT descripcion AS nombre FROM tm_tipo_pago WHERE id_tipo_pago = ?", $d->id_tpag);

                $c[$k]->{'Cliente'} = $cliente;
                $c[$k]->{'Tipopago'} = $tipopago;
            }
    ...
    public function venta_all_imp_($data)
    {
        try
        {
            ...
            $cliente_stm = $this->db->prepare("SELECT * FROM v_clientes WHERE id_cliente = ?");
            $cliente_stm->execute(array($venta->id_cli));
            $venta->Cliente = $cliente_stm->fetch(PDO::FETCH_OBJ);

            $pedido_stm = $this->db->prepare("SELECT vm.desc_salon, vm.nro_mesa  FROM tm_pedido_mesa AS pm INNER JOIN v_mesas AS vm ON pm.id_mesa = vm.id_mesa WHERE pm.id_pedido = ?");
            $pedido_stm->execute(array($venta->id_ped));
            $venta->Pedido = $pedido_stm->fetch(PDO::FETCH_OBJ);

            $detalle_stm = $this->db->prepare("SELECT v_productos.pro_cod AS codigo_producto, CONCAT(v_productos.pro_pre) AS nombre_producto, IF(v_productos.pro_imp='1','10','20') AS codigo_afectacion, CAST(tm_detalle_venta.cantidad AS DECIMAL(7,2)) AS cantidad, IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ?)),2),tm_detalle_venta.precio) AS valor_unitario, tm_detalle_venta.precio AS precio_unitario, IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ?))*tm_detalle_venta.cantidad,2),ROUND(tm_detalle_venta.precio*tm_detalle_venta.cantidad,2)) AS valor_venta, IF(v_productos.pro_imp='1',ROUND((tm_detalle_venta.precio/(1 + ?)*tm_detalle_venta.cantidad)*?,2),0) AS total_igv FROM tm_detalle_venta INNER JOIN tm_venta ON tm_detalle_venta.id_venta = tm_venta.id_venta INNER JOIN v_productos ON tm_detalle_venta.id_prod = v_productos.id_pres WHERE tm_venta.id_tipo_doc IN ('1','2','3') AND tm_detalle_venta.precio > 0 AND tm_detalle_venta.id_venta = ?");
            $detalle_stm->execute(array($this->igv_universal, $this->igv_universal, $this->igv_universal, $this->igv_universal, $data));
            $venta->Detalle = $detalle_stm->fetchAll(PDO::FETCH_OBJ);

            if (empty($venta->Detalle)) {
                $consulta = $this->db->prepare("SELECT * FROM tm_detalle_venta WHERE id_prod = '111111' AND id_venta= ?");
                $consulta->execute(array($data));
                $resultados = $consulta->fetchAll(PDO::FETCH_ASSOC);

                if (empty($resultados)) {
                    $venta->Detalle = [(object)[
                        "codigo_producto" => "111111",
                        "nombre_producto" => "POR CONSUMO",
                        "codigo_afectacion" => (Session::get('type_operation') == 1) ? 10 : 20,
                        "cantidad" => "1.00",
                        "valor_unitario" => "",
                        "precio_unitario" => "",
                        "valor_venta" => "",
                    ...
                    foreach ($resultados as $resultado) {
                        $detalle = [
                            ...
                            "nombre_producto" => "POR CONSUMO",
                            "codigo_afectacion" => (Session::get('type_operation') == 1) ? 10 : 20,
                            "cantidad" => number_format($resultado["cantidad"], 2),
                            "valor_unitario" => number_format($resultado["precio"] / (1 + $this->igv_universal), 2),
                            "precio_unitario" => number_format($resultado["precio"], 2),
                            "valor_venta" => number_format($resultado["precio"] / (1 + $this->igv_universal) * $resultado["cantidad"], 2),
                            "total_igv" => number_format(($resultado["precio"] / (1 + $this->igv_universal) * $resultado["cantidad"]) * $this->igv_universal, 2)
                        ];

                        $detalles[] = (object)$detalle;
                    }
    ...
    public function finanza_ing_list($data)
    {
        try
        {
            ...
            $stm = $this->db->prepare("
                SELECT ia.*, ca.desc_caja, CONCAT(u.nombres, ' ', u.ape_paterno, ' ', u.ape_materno) AS desc_usu
                FROM tm_ingresos_adm AS ia
                LEFT JOIN v_caja_aper AS ca ON ia.id_apc = ca.id_apc
                LEFT JOIN tm_usuario AS u ON ia.id_usu = u.id_usu
                WHERE (DATE(ia.fecha_reg) >= ? AND DATE(ia.fecha_reg) <= ?)
                AND ia.id_usu LIKE ?
                AND ia.estado LIKE ?
            ");
    ...
    public function oper_anul_list($data)
    {
        try
        {
            ...
            $stm = $this->db->prepare("
                SELECT dp.*, (dp.cant * dp.precio) AS total, 
                    CONCAT(u.ape_paterno, ' ', u.ape_materno, ' ', u.nombres) AS nombres,
                    tp.id_tipo_pedido,
                    p.*
                FROM tm_detalle_pedido AS dp
                LEFT JOIN tm_usuario AS u ON dp.id_usu = u.id_usu
                LEFT JOIN tm_pedido AS tp ON dp.id_pedido = tp.id_pedido
                LEFT JOIN v_productos AS p ON dp.id_pres = p.id_pres
                WHERE (DATE(dp.fecha_pedido) >= ? AND DATE(dp.fecha_pedido) <= ?)
                    AND dp.id_usu LIKE ?
                    AND dp.estado = 'z'
            ");
    ...
    public function finanza_arq_resumen_imp($data)
    {
        try
        {      
            $stm = $this->db->prepare("SELECT * FROM v_caja_aper WHERE id_apc = ?");
